...
class WeatherViewController: UIViewController, CLLocationManagerDelegate, ChangeCityDelegate {
    ...
    let APP_ID = "c436ed0963e228f49fa17fa7ef71aa6b"
    ...
    func getWeatherData(url: String, parameter: [String: String]) {
        Alamofire.request(url, method: .get, parameters: parameter).responseJSON {
            ...
            if response.result.isSuccess {
                ...
                print("Kode error: \(response.result.error!)")
    ...
    func updateWeatherData(json: JSON) {
        if let tempResult = json["main"]["temp"].double {
            ...
            cityLabel.text = "lagi gak bisa coy"
        }
    }
    ...
    func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
        print(error)
        cityLabel.text = "gak dapet sinyal gps coy"
    }
